...
class TestRMDIRCommand(unittest.TestCase):
    ...
    def test_rmdir_success(self, mock_get_context, mock_send, _mock_is_valid, _mock_is_expired):
    ...
    @patch("skylock_cli.core.dir_operations.send_rmdir_request")
    @patch("skylock_cli.core.context_manager.ContextManager.get_context")
    def test_rmdir_success_chnage_dir(self, mock_get_context, mock_send_rmdir, mock_send_cd, _mock_save_context, _mock_is_valid, _mock_is_expired):
        """Test the rmdir command"""
        ...
        result = runner.invoke(app, ["rmdir", "/test_dir/"])
        self.assertEqual(result.exit_code, 0)
        mock_send_rmdir.assert_called_once_with(mock_get_context.return_value.token, Path("/test_dir"), False)
        self.assertIn("Current working directory: /", result.output)
        self.assertIn("Directory /test_dir removed successfully", result.output)
    ...
    @patch("skylock_cli.core.dir_operations.send_rmdir_request")
    @patch("skylock_cli.core.context_manager.ContextManager.get_context")
    def test_rmdir_success_recursive_long(self, mock_get_context, mock_send, _mock_is_valid, _mock_is_expired):
        """Test the rmdir command"""
        mock_get_context.return_value = mock_test_context()
    ...
    @patch("skylock_cli.core.dir_operations.send_rmdir_request")
    @patch("skylock_cli.core.context_manager.ContextManager.get_context")
    def test_rmdir_success_recursive_short(self, mock_get_context, mock_send, _mock_is_valid, _mock_is_expired):
        """Test the rmdir command"""
        mock_get_context.return_value = mock_test_context()
