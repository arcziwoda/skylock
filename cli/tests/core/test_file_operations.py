...
from skylock_cli.core.file_operations import _generate_unique_file_path, upload_file, download_file
...
class TestUploadFile(unittest.TestCase):
    ...
    def test_upload_file_with_other_than_cwd_destination_path(self, _mock_send, _mock_get_context):
    ...
    def test_upload_file_already_exists(self, _mock_send, _mock_get_context):
        ...
        with tempfile.NamedTemporaryFile() as temp_file:
            ...
            with patch("sys.stderr", new_callable=StringIO) as mock_stderr:
                ...
                self.assertIn(f"File `/{temp_file_name}` already exists!", mock_stderr.getvalue())
    ...
    def test_upload_file_dest_path_does_not_exists(self, _mock_send, _mock_get_context):
        ...
        with tempfile.NamedTemporaryFile() as temp_file:
            ...
            with patch("sys.stderr", new_callable=StringIO) as mock_stderr:
                ...
                self.assertIn(f"Invalid path `/{temp_file_name}`!", mock_stderr.getvalue())
    ...
    def test_upload_file_with_internal_server_error(self, _mock_send, _mock_get_context):
        """Test the upload_file function with an unauthorized error"""
        with tempfile.NamedTemporaryFile() as temp_file:
            temp_file_path = Path(temp_file.name)
            with patch("sys.stderr", new_callable=StringIO) as mock_stderr:
                ...
                self.assertIn("Failed to upload file (Error Code: 500)", mock_stderr.getvalue())
...
class TestDownloadFile(unittest.TestCase):
    ...
    @patch("skylock_cli.core.file_operations._generate_unique_file_path", return_value=Path("/mocked/Downloads/file.txt"))
    @patch("skylock_cli.core.context_manager.ContextManager.get_context", return_value=mock_test_context())
    @patch("skylock_cli.core.file_operations.send_download_request", return_value=b"123\nabc")
    @patch("skylock_cli.core.file_operations.Path.exists", return_value=True)
    @patch("builtins.open", new_callable=mock_open)
    def test_download_file_target_path_exists(
        self, _mock_open, _mock_exists, _mock_send, _mock_get_context, _mock_generate_unique, _mock_create_downloads_dir
    ):
    ...
    @patch("skylock_cli.core.file_operations.DOWNLOADS_DIR", new=Path("/mocked/Downloads/"))
    @patch("skylock_cli.core.file_operations.create_downloads_dir", return_value=None)
    @patch("skylock_cli.core.context_manager.ContextManager.get_context", return_value=mock_test_context())
    @patch("skylock_cli.core.file_operations.send_download_request", return_value=b"123\nabc")
    @patch("skylock_cli.core.file_operations.Path.exists", return_value=False)
    @patch("builtins.open", new_callable=mock_open)
    def test_download_file_target_path_does_not_exist(self, _mock_open, _mock_exists, _mock_send, _mock_get_context, _mock_create_downloads_dir):
    ...
    @patch("skylock_cli.core.context_manager.ContextManager.get_context", return_value=mock_test_context())
    @patch("skylock_cli.api.file_requests.client.get", return_value=mock_response_with_status(HTTPStatus.UNAUTHORIZED))
    ...
    @patch("skylock_cli.core.context_manager.ContextManager.get_context", return_value=mock_test_context())
    @patch("skylock_cli.api.file_requests.client.get", return_value=mock_response_with_status(HTTPStatus.BAD_REQUEST))
    ...
    @patch("skylock_cli.core.context_manager.ContextManager.get_context", return_value=mock_test_context(Path("/test")))
    @patch("skylock_cli.api.file_requests.client.get", return_value=mock_response_with_status(HTTPStatus.NOT_FOUND))
    ...
    def test_download_file_with_invalid_response_format_error(self, mock_send, _mock_get_context):
