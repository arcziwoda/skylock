...
from skylock_cli.api.resource_requests import send_mkdir_request
...
def create_directory(directory_name: str, parent: bool) -> Path:
    """Create a directory in the user's cwd"""
    current_context = context_manager.ContextManager.get_context()
    directory_path = path_parser.parse_path(current_context.user_dir.path, Path(directory_name))
    with APIExceptionHandler():
        send_mkdir_request(current_context.token, directory_path, parent)
